...
\end{figure}
Figure \ref{fig:phaseport-3.1-3.1} shows the first appearance of a bifurcation in the phase portrait.
...
As expected for the symmetric case $r_1 = r_2$, both points lie on the diagonal with $x=y$.
\newpage
...
\end{figure}
When further increasing $r_2$, we reach the chaotic region of $r$ for the simple logistic map.
The impact on the investigated coupled map is shown in Figure \ref{fig:phaseport-3.1-3.95}.
...
The sizes of the regions vary significantly, the smallest is in the top right corner at around $(x, y) = (0.84, 0.92)$ and the largest region as at approximately $(x, y) = (0.53, 0.54)$ with a spread of around $0.05$ in both coordinates, slightly more so in the $y$-direction than the $x$-direction.
\newpage
...
\end{figure}
The best picture of a chaotic behaviour is Figure \ref{fig:phaseport-3.85}.
...
The shape is oriented almost along the diagonal but not quite because the scaling parameters $r_1$ and $r_2$ are once again chosen to be slightly different, however, both in the chaotic regime.
\clearpage