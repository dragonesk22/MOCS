...
def plot_phase_portrait(x0y0,
    ...
    plt.scatter(*traj.T,
                ...
                label=f"x0y0=({x0y0[0]:.3f}, {x0y0[1]:.3f})," +
                f"(r1,r2,$\\epsilon$)={params}")
...
def get_report_figures():
    ...
    for param in [(2.8, 2.9, 0.1), (3.1, 3.4, 0.3), (3.85, 3.95, 0.2),
                  (3.1, 3.1, 0.3), (3.1, 3.95, 0.3)]:
        print(f"\n{param=}")
        for _ in range(10):
            plot_phase_portrait(rng.random(2),
                                param,
                                trans=trans_val,
                                should_print=True)
...
if __name__ == "__main__":
    trans_val = 20
    plot_phase_portrait(rng.random(2), default_params, trans=trans_val)
    plot_phase_portrait(rng.random(2), default_params, trans=trans_val)
    plt.legend()
    plt.colorbar()
    plt.show()
